...
final class MainViewController: UIViewController {
    ...
    private lazy var segmentedControl: UISegmentedControl = {
        let segment = UISegmentedControl(
            items: [
                CollectionViewNamespace.list.name,
        ...
        segment.setTitleTextAttributes(
            [NSAttributedString.Key.foregroundColor: UIColor.secondary ?? UIColor.black],
    ...
    private func setCollectionViewConstraint() {
        NSLayoutConstraint.activate([
            ...
            addedButton.bottomAnchor.constraint(
                equalTo: view.safeAreaLayoutGuide.bottomAnchor,
                constant: -20
            ),
            addedButton.trailingAnchor.constraint(
                equalTo: view.safeAreaLayoutGuide.trailingAnchor,
                constant: -35
            )
        ])
    ...
    private func setCollectionView() {
        ...
        dataSource = configureDataSource(
            id: CollectionViewNamespace.list.name
        )
    }
        ...
        dataSource = DiffableDataSource(collectionView: collectionView) {(
            ...
            case CollectionViewNamespace.list.name:
                guard let cell = collectionView.dequeueReusableCell(
                    withReuseIdentifier: CollectionViewNamespace.list.name,
                    for: indexPath
            ...
            default:
                guard let cell = collectionView.dequeueReusableCell(
                    withReuseIdentifier: CollectionViewNamespace.grid.name,
                    for: indexPath
    ...
    private func createListLayout() -> UICollectionViewLayout {
        let itemSize = NSCollectionLayoutSize(
            widthDimension: .fractionalWidth(Metric.listItemWidth),
            heightDimension: .fractionalHeight(Metric.listItemHeight)
        )
        let item = NSCollectionLayoutItem(layoutSize: itemSize)
        
        let groupSize = NSCollectionLayoutSize(
            widthDimension: .fractionalWidth(Metric.listGroupWidth),
    ...
    private func createGridLayout() -> UICollectionViewCompositionalLayout {
        let itemSize = NSCollectionLayoutSize(
            widthDimension: .fractionalWidth(Metric.gridItemWidth),
            heightDimension: .fractionalHeight(Metric.gridItemHeight)
        )
        let item = NSCollectionLayoutItem(layoutSize: itemSize)
        
        let groupSize = NSCollectionLayoutSize(
            widthDimension: .fractionalWidth(Metric.gridGroupWidth),
    ...
    private func moveProductRegistrationPage() {
        let registrationViewController = RegistrationEditViewController(
            viewModel: RegistrationEditViewModel(marketItem: nil)
        )
...
extension MainViewController: UICollectionViewDelegate {
    func collectionView(
        ...
        didSelectItemAt indexPath: IndexPath) {
            ...
            viewModel.input.pushToDetailView(
                indexPath: indexPath,
                id: id
            )
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if Int(currentPosition) == Int(bottomPosition) {
            self.loadingView.startAnimating()
            viewModel.input.increaseProductPageNumber()
            viewModel.input.getInformation()
        }
        
        if scrollView.contentOffset.y > 0 {
            addedButton.setImage(
                UIImage(named: "addedButton2"),
                for: .normal
            )
